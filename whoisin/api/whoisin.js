...
export default async function handler(req, res) {
  // Handle POST requests that are actually GET (used for complex nested selects)
  if (req.method === 'POST' && req.headers['x-sb-method'] === 'GET') {
    const { table: bodyTable, select: bodySelect, filter: bodyFilter, order: bodyOrder, limit: bodyLimit } = req.body || {};
    req.method = 'GET';
    req.query = req.query || {};
    if (bodyTable) req.query.table = bodyTable;
    if (bodySelect) req.query.select = bodySelect;
    if (bodyFilter) req.query.filter = bodyFilter;
    if (bodyOrder) req.query.order = bodyOrder;
    if (bodyLimit) req.query.limit = bodyLimit;
  }
